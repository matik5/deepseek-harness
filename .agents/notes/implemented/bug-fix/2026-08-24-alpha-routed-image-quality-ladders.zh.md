...
本决定部分取代[统一图片请求管线记录](../feature/2026-08-20-unified-image-request-pipeline.zh.md)中的持久规范化设计。模型请求投影为不支持 WebP 解码的端点使用 JPEG。
...
- 阶梯耗尽的持久附件可能以超过字节目标的大小落盘；实测只有对抗性随机 alpha 输入可达。把这样的超目标 master 再次作为新上传提交时，直通的字节检查不通过，会再走一遍有损阶梯，因此规范化对这一仅对抗性可达的类别不幂等，每轮都会累积代际损失。
